...
def _safe_wandb_log(data):
    """Log to wandb only if a run is active. Never crashes training."""
    try:
...
class NeuralNetwork:
    ...
    def forward(self, X):
        """
        Forward propagation. Returns raw logits for the output layer.
        """
        A = X
        for i, layer in enumerate(self.layers):
            Z = layer.forward(A)
            if i == len(self.layers) - 1:
                return Z  # raw logits, no activation on output
            A = layer.activation_fn.forward(Z)
            layer.A_cache = A
        return A
    ...
    def train(self, X_train, y_train, epochs, batch_size):
        ...
        for epoch in range(epochs):
            ...
            for j in range(0, num_samples, batch_size):
                ...
                # Softmax for loss computation only
                probs = Softmax().forward(logits)
                ...
                # Backward — pass logits; backward applies softmax internally
                self.backward(y_batch, logits)

                # Safe wandb gradient logging
                layer_0_grad_W = self.layers[0].grad_W
                grad_logs = {
                    f"grad_neuron_{i}": np.mean(np.abs(layer_0_grad_W[:, i]))
                    for i in range(min(5, layer_0_grad_W.shape[1]))
                }
                grad_logs["grad_norm_first_layer"] = np.linalg.norm(layer_0_grad_W)
                _safe_wandb_log(grad_logs)

                self.update_weights()

            avg_loss = total_loss / (num_samples // batch_size)
            _safe_wandb_log({"epoch": epoch + 1, "train_loss": avg_loss})
            print(f"Epoch {epoch+1}/{epochs} - Loss: {avg_loss:.4f}")
